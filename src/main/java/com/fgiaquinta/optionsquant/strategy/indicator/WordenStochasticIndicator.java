package com.fgiaquinta.optionsquant.strategy.indicator;
 ...
 * Calculates the percentile rank of the current close price within a lookback period.
 * Used by C5/P5 (Efecto Imán) strategies for confirmation signal.
 */
public class WordenStochasticIndicator extends CachedIndicator<Num> {
    ...
    protected Num calculate(int index) {
        ...
        // Collect all close prices within the specified lookback period
        for (int i = index - period + 1; i <= index; i++) {
            historicalCloses.add(closePrice.getValue(i).doubleValue());
        }

        // Sort the prices to determine the Percentile Rank
        ...
        // Apply the Worden formula: (100 / (n - 1)) * Rank
